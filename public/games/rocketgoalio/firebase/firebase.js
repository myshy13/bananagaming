  ...
  while (!hasFetchedConfig) {
    try {
      ...
      if (timeSinceLastFetch <= remoteConfig.settings.minimumFetchIntervalMillis + 1000 * 60 * 2) {
        ...
        console.error(
          "Remote config is stale with status: " +
            remoteConfig.lastFetchStatus +
            ". (time since last succesful fetch: " +
            timeSinceLastFetch +
            " milliseconds), trying to fetch again",
        )
      }
    } catch (error) {
...
async function linkEmailPasswordProvider(email, password, isRegistration = false) {
  try {
    ...
    if (!isRegistration) {
      ...
      if (accountExists) {
        throw new Error(
          "Account already exists. Please use login instead or choose a different username.",
        )
      }
    ...
    if (
      await handleRequiresRecentLogin(
        error,
        () => linkEmailPasswordProvider(email, password, isRegistration),
        "RumbleRush",
        "link",
      )
    ) {
    ...
    try {
      ...
      gameInstance.SendMessage(
        ...
        JSON.stringify({
          success: false,
          error:
            "Recent authentication required. Please log out and log in again, then try linking your account.",
          errorCode: error.code,
          operation: operation,
        }),
      )
